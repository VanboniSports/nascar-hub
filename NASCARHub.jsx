...
function getTier(r) {
  ...
  if (r >= 63) return { bg:"rgba(30,60,90,0.06)",   border:T.border2 };
  return         { bg:"rgba(20,40,60,0.04)",         border:T.border };
}
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (csvRaw) {
          // Re-normalize driver names in case cached data used old normalizer
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'Barlow',sans-serif", color:T.text, display:"flex", flexDirection:"column" }}>
        ...
        <footer style={{ borderTop:`1px solid ${T.border}`, padding:"7px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", fontSize:10, color:T.textDim, fontFamily:"'IBM Plex Mono',monospace", background:T.footerBg }}>
          ...
          <span>{raceArchive.length} race{raceArchive.length!==1?"s":""} archived · {battleRaces.length} battle race{battleRaces.length!==1?"s":""} · {csvData.length > 0 ? `${csvData.length} CSV records` : "no CSV"} · {drivers.length} drivers</span>
